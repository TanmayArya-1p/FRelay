...
#TIMEOUT FEATURE DIABLEDMANUALLY for now
class RouteManager():
	def __init__(self,poolSize : int = 10,route_pool : list = [] ,timeout : int = 5 , maxFileSize = math.inf,autopopulate=True , autopopulateconfig={'poolUpperLimit':25,'threshold':0.7}):
	...
	def flush(self):
		...
		files = glob.glob(os.getcwd()+r"/tmp/*")
		for f in files:
			os.remove(f)
		self.populatePool(self.poolSize)
